...
def calc_refl_457(np_image, band_num, meta_path):
    ...
    if not isinstance(meta_path, str):
        raise ValueError(
            "calc_refl_457 requires a str type for meta_path, not metadata"
        )
    ...
    dSun2 = (
        ...
        + 0.001280 * math.sin(theta)
        + 0.000719 * math.cos(2 * theta)
        + 0.000077 * math.sin(2 * theta)
    )

    SZA = 90.0 - float(getattr(metadata, "SUN_ELEVATION"))
...
def calc_reflc_8(np_image, band_num, meta):
    ...
    Mp = getattr(
        meta, "REFLECTANCE_MULT_BAND_{0}".format(band_num)
    )  # multiplicative scaling factor
    Ap = getattr(
        meta, "REFLECTANCE_ADD_BAND_{0}".format(band_num)
    )  # additive rescaling factor
    SEA = getattr(meta, "SUN_ELEVATION") * (
        math.pi / 180
    )  # sun elevation angle theta_se
...
def toa_reflectance_457(band_nums, meta_path):
    ...
    for band_num in band_nums:
        ...
        Qcal = Qcal.astype(np.float32)
        Qcal[hit] = np.nan
        Ref_raster = calc_refl_457(Qcal, band_num, meta_path)
        out_dict[int(band_num)] = Ref_raster
    return out_dict
